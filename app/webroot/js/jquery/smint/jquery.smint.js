		...
		return $('.smint a').each( function(index) {
			...
			var stickyMenu = function(direction){
				...
				if(optionLocs[index][0] <= scrollTop && scrollTop <= optionLocs[index][1]){	
					if(direction == "up"){
						...
						$("#"+optionLocs[index+1][2]).parent().removeClass("actif");
					} else if(index > 0) {
						$("#"+id).parent().addClass("actif");
						$("#"+optionLocs[index-1][2]).parent().removeClass("actif");
					...
					$.each(optionLocs, function(i){
						if(id != optionLocs[i][2]){
							console.log(i);
							$("#"+optionLocs[i][2]).parent().removeClass("actif");
						}
					});
				}
			};
			...
			$(window).scroll(function() {
				...
				if($(window).scrollTop() + $(window).height() == $(document).height()) {
       			$('.smint a').parent().removeClass('actif')
       			$('.smint a').parent().last().addClass('actif')
   }
			});
