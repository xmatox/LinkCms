	...
	$.fn.dropfile = function(oo){
		...
		function select(div,img){
			...
			xhr3.addEventListener("load",function(e){
				...
				var id = $("#GaleriesphotoelementId").val();
				var formUrl = __prefix+"/galeriesphotos/galeriesphotoelements/ajax_getselect/";
				$.ajax({
					type: 'POST',
					url: formUrl,
					async: true,
					dataType: 'json',
					data: { id: id, selection: json.name },
